/*
 * Jun Liao
 * CPSC 5011 02 21WQ Object-Oriented Concepts, Seattle University
 * This is homework for week 4
 */
...
/**
 * The Driver interfaces with the registration system and
 * populates faculty, students, subjects, courses, their prerequisites, and
 * course sections.
 * It also test handling all exceptions.
 * @author Jun Liao
 * @version 1.0
 */
public class Driver {
    ...
    private static void populateStudents(RegistrationSystem system) throws DuplicatePersonException {
        ...
        system.addStudent("Gob", "Bluth", StudentType.UNDERGRAD, StudentProgram.BACS, Quarter.SQ, 2019);
        system.addStudent("Buster", "Bluth", StudentType.UNDERGRAD, StudentProgram.BSCS, Quarter.WQ, 2020);
        system.addStudent("Lucille", "Bluth", StudentType.UNDERGRAD, StudentProgram.BSCS, Quarter.WQ, 2020);
        system.addStudent("Lindsay", "Funke", StudentType.UNDERGRAD, StudentProgram.BACS, Quarter.FQ, 2019);
        system.addStudent("Tobias", "Funke", StudentType.UNDERGRAD, StudentProgram.BACS, Quarter.FQ, 20120);
        system.addStudent("Maeby", "Funke", StudentType.UNDERGRAD, StudentProgram.BSCS, Quarter.SQ, 2021);
    ...
    private static void populateSections(RegistrationSystem system) throws CourseNotFoundException, PersonNotFoundException, DuplicateSectionException {
    ...
    /***
     * Assigns every student a advisor
     * Prints all students, faculties, subjects, courses, sections in the system instance
     * Tests handling all exceptions
     * @param system
     */
    private static void printSystem(RegistrationSystem system)  {
        // TODO: implement printSystem method

        // Randomly assign a student a advisor from the faculty list, so that we can print
        system.assignAdvisorForAllStudents();

        // Print the system's all fields information as required output
        // including all students, faculties, subjects, courses, sections information
        System.out.println(system.toString());

        // Test faculty not found exception
        try {
            system.retrieveFacultyName("Jun Liao");
        } catch (PersonNotFoundException e) {
            System.out.println(e.getMessage());
        }
        ...
        try {
            system.retrieveStudentName("Jun Liao");
        }catch (PersonNotFoundException e) {
            System.out.println(e.getMessage());
        }

        // Test course not found exception
        try {
            system.retrieveCourseCodeAndNumber(SubjectCode.CPSC, 1234);
        } catch (CourseNotFoundException e) {
            System.out.println(e.getMessage());
        }

        // Test section not found exception
        try {
            system.retrieveSection(SubjectCode.CPSC, 1111, 1);
        }
        catch (SectionNotFoundException e) {
            System.out.println(e.getMessage());
        }

        // Test duplicate faculty exception
        try {
            system.addFaculty("Adair", "Dingle", FacultyType.PROF, Building.ENGR, 531, "dingle@seattleu.edu");
        } catch (DuplicatePersonException e) {
            System.out.println(e.getMessage());
        ...
        // Test duplicate student exception
        try {
            system.addStudent("Michael", "Bluth", StudentType.UNDERGRAD, StudentProgram.BSCS, Quarter.FQ, 2018);
        } catch (DuplicatePersonException e) {
            System.out.println(e.getMessage());
        ...
        // Test duplicate course exception
        try {
            system.addCourse(SubjectCode.CPSC, 1420, "Programming and Problem Solving I", 5);
        } catch (DuplicateCourseException e) {
            System.out.println(e.getMessage());
        ...
        // Test duplicate section exception
        try {
            system.addSection(SubjectCode.CPSC, 2430, 1, "Susan", "Reeder", Quarter.FQ, 2018, 15, Building.PIGT, 207);
        } catch (DuplicateSectionException | CourseNotFoundException | PersonNotFoundException e) {
            System.out.println(e.getMessage());
        }
